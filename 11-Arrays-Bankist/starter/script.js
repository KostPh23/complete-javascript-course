...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // 3. For each remaining dog, "Dog number 1 is an adult, and is 5 years old" or "Dog number 2 is still a puppy 🐶"
...
const calcAverageHumanAge = function (ages) {
  return ages
    ...
    .reduce(
      (acc, humanAge, _, humanAges) => acc + humanAge / humanAges.length,
      0
    );
};
console.log('---- TEST DATA 1 ----');
...
const accountFilter = accounts.filter(acc => acc.owner === 'Jessica Davis')[0];
...
const { depositsReduce, withdrawalsReduce } = accounts
  .reduce((arr, cur) => {
    // map
    arr.push(cur.movements);
    return arr;
  }, [])
  .reduce((arr, cur) => {
    // flat
    // for (const elem of cur) arr.push(elem);
    cur.reduce((arr, cur) => {
      // forEach
      arr.push(cur);
      return arr;
  ...
  .reduce(
    // Compute object of sums of deposits and withdrawals
    (sums, cur) => {
      // cur > 0 ? (sums.deposits += cur) : (sums.withdrawals += cur);
      sums[cur > 0 ? 'depositsReduce' : 'withdrawalsReduce'] += cur;
      return sums;
    },
    {
      depositsReduce: 0,
...
console.log(depositsReduce, withdrawalsReduce);
...
console.log(convertTitleCase('this TiTle has an and in its SENTENCE'));
*/
...
console.log(`${joinArray(ownersEatTooMuch)}'s dogs eat too much!`);
console.log(`${joinArray(ownersEatTooLittle)}'s dogs eat too little!`);
